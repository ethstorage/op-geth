...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s WSCs", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
...
func newFaucet(rpcFlag, ethRpcFlag string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
...
func authTwitter(url string, tokenV1, tokenV2 string) (string, string, string, common.Address, error) {
	...
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", "", common.Address{}, errors.New("No WSC Chain address found to fund")
	}
	var avatar string
...
func authNoAuth(ethClient *ethclient.Client, url string) (string, string, common.Address, error) {
	...
	if bal.Cmp(minEthNoAuthAddrNeed) < 0 {
		return "", "", common.Address{}, errors.New(
			fmt.Sprintf("Your address %s need to have at least 0.01 eth in Ethereum mainnet.", address.Hex()))
	}
